...
[Arduino ENS160+AHT21] → USB Serial → [MCP Server] → [Sensor Loop]
                                                          ↓
Dashboard → A2A message/send → [Mac ADK Agent] ← REST push ← [Jetson ADK Agent]
                                    ↑                                    ↑
                              REST /api/*                          /.well-known/agent-card.json
```

**Key flow:**
- **Chat**: Dashboard → A2A `message/send` (JSON-RPC) → Mac → (delegates via sub_agents) → Jetson
- **Data**: Sensor loop → REST push → Mac → REST `/api/*` → Dashboard
...
4. **Background tasks** — sensor loop (Jetson), historical data loading (Mac)
...
- Mac agent: 8081 (A2A + REST + WebSocket)
